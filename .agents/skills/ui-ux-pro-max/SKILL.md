...
- **Correct brand logos**: Research official SVG from Simple Icons; don't guess logo paths.
...
- **Smooth transitions**: Use `transition-colors duration-200` (150-300ms).

### Light/Dark Mode Contrast
- **Glass card light mode**: Use `bg-white/80` or higher opacity.
- **Text contrast light**: Use `#0F172A` (`slate-900`) for primary text, `#475569` (`slate-600`) minimum for muted text.
...
- **Consistent max-width**: Use same `max-w-6xl` or `max-w-7xl` container widths across sections.
